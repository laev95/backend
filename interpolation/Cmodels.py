import ctypes       as ct
from itertools      import repeat
from .spacebuffer   import space_buffer


class CGeoValuesArray:
    def __init__(self, x_coords, y_coords, elli_hs, norm_hs, n_values, inter_type, coord_type):
        self.x_arr      = (ct.c_double * n_values)(*x_coords)
        self.y_arr      = (ct.c_double * n_values)(*y_coords)
        self.eh_arr     = (ct.c_double * n_values)(*elli_hs)
        self.nh_arr     = (ct.c_double * n_values)(*norm_hs)

        self.i_type     = space_buffer(inter_type)
        self.system     = space_buffer(coord_type)

        self.ih_arr     = (ct.c_double * n_values)(*list(repeat(0, n_values)))
        self.num_el     = ct.c_int(n_values)
        self.int_flags  = (ct.c_int * n_values)(*list(repeat(0, n_values)))
        self.err_flag   = ct.c_int(0)

    def convert_to_python(self) -> list:
        fortran_values:     list[list] = []
        geo_list:           list[dict] = []

        number_elems = self.num_el.value

        fortran_values.append(list(self.x_arr))
        fortran_values.append(list(self.y_arr))
        fortran_values.append(list(self.eh_arr))
        fortran_values.append(list(self.nh_arr))
        fortran_values.append(list(repeat(self.i_type.decode().strip(), number_elems)))
        fortran_values.append(list(repeat(self.system.decode().strip(), number_elems)))
        fortran_values.append(list(self.ih_arr))
        fortran_values.append(list(self.int_flags))
        fortran_values.append(list(repeat(self.err_flag.value, number_elems)))

        dict_keys = [   
                    "x_coord", 
                    "y_coord",

                    "ellips_height", 
                    "normal_height",

                    "inter_type", 
                    "coord_type",

                    "inter_height",

                    "int_flag",
                    "err_flag"  
                    ]
        
        for i in range(number_elems):
            geo_list.append(dict(zip(dict_keys, [value_list[i] for value_list in fortran_values])))

        return geo_list
